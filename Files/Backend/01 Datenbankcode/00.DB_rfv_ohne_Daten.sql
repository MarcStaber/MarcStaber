...
-- Exportiere Daten aus Tabelle rfv.court: ~0 rows (ungefähr)
DELETE FROM `court`;
...
CREATE TABLE IF NOT EXISTS `court_reservation` (
  ...
  `reservation_datetime` datetime NOT NULL COMMENT 'Rerservierungszeit',
...
-- Exportiere Daten aus Tabelle rfv.reservation_type: ~0 rows (ungefähr)
...
-- Exportiere Daten aus Tabelle rfv.role: ~0 rows (ungefähr)
DELETE FROM `role`;
...
-- Exportiere Daten aus Tabelle rfv.user: ~0 rows (ungefähr)
DELETE FROM `user`;
